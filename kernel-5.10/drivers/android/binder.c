                    ...
                    struct binder_thread *thread)
{
    ...
    struct binder_priority node_prio;
    bool oneway = !!(t->flags & TF_ONE_WAY); // false
                   ...
                   binder_size_t extra_buffers_size)
{
    ...
    if (reply) {
        binder_inner_proc_lock(proc);
        in_reply_to = thread->transaction_stack; // 保存的就是这次通信的事务 binder_transaction
        if (in_reply_to == NULL) {
            binder_inner_proc_unlock(proc);
        ...
        thread->transaction_stack = in_reply_to->to_parent; // 取出下一个事务
        binder_inner_proc_unlock(proc);
        // 下面方法主要就是这次通信事务 transation_stack->from 得到之前请求进程的 binder_thread，从而得到目标进程的 binder_proc
    ...
    } else { // 第四个参数为 false，进入 else 分支，处理 BC_TRANSACTION
 ...
 retry_alloc:
    ...
    // 把数据从用户空间拷贝到上面的 buffer 共享内存区域，即 binder 真正一次拷贝有效数据的地方
    // 拷贝用户空间的 tr->data.ptr.buffer 到 t->buffer 对应的物理内存
    // 拷贝的是 transact() 中 data 参数的非 IBinder 数据
    ...
    // 拷贝的是 transact() 中 data 参数的 IBinder 对象
    if (binder_alloc_copy_user_to_buffer(
                &target_proc->alloc,
                t->buffer,
    ...
    buffer_offset = off_start_offset; // 数据中 IBinder 对象的起始地址，data_size 为非 IBinder 对象数据
    off_end_offset = off_start_offset + tr->offsets_size; // 数据中 IBinder 对象的结束地址
    ...
    for (buffer_offset = off_start_offset; buffer_offset < off_end_offset;
        ...
        struct binder_object object;
        binder_size_t object_offset;
        // 从 buffer 中拷贝以 buffer_offset 地址开始 sizeof(object_offset) 大小的数据到 object_offset 地址中
        ...
        // 从 buffer 中获取 object 并返回 object_size
    ...
    if (reply) {
        ...
        binder_inner_proc_unlock(proc);
        // 此时 target_thread 还是 NULL，进去后会从 target_proc 的 waiting_threads
        ...
        binder_enqueue_thread_work(thread, tcomplete); // 把 tcomplete 添加到 thread->todo 里发送给 Client 端
...
retry:
    ...
    binder_inner_proc_unlock(proc);
    thread->looper |= BINDER_LOOPER_STATE_WAITING; // 进入 WAITING 状态，0x10
    ...
    thread->looper &= ~BINDER_LOOPER_STATE_WAITING; // 解除等待状态
    if (ret)
        return ret;
    while (1) {
        uint32_t cmd;
...
skip:
        ...
        case BINDER_WORK_TRANSACTION_ONEWAY_SPAM_SUSPECT: {
            ...
            kfree(w);
            binder_stats_deleted(BINDER_STAT_TRANSACTION_COMPLETE);
            if (put_user(cmd, (uint32_t __user *)ptr)) // 把命令写入用户空间的 ptr
                return -EFAULT;
            ptr += sizeof(uint32_t);
            binder_stat_br(proc, thread, cmd);
        ...
        if (!t)
            continue; // t 为 NULL 时直接 continue
        ...
        if (t_from)
            binder_thread_dec_tmpref(t_from);
        // 表示事务 t 的 buffer 允许目标线程在用户空间发出 BC_FREE_BUFFER 命令协议来释放
