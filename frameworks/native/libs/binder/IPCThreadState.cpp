...
restart:
        ...
        IPCThreadState* st = (IPCThreadState*)pthread_getspecific(k); // 从线程私有空间获取
        if (st) return st;
...
status_t IPCThreadState::talkWithDriver(bool doReceive)
{
    ...
    bwr.write_size = outAvail; // // 要写入的数据量，mOut.dataSize() > 0
    ...
    if (err >= NO_ERROR) {
        if (bwr.write_consumed > 0) { // binder 驱动消耗了 mOut 中的数据，成功写入了数据
            // 比较已消费的数据量和 mOut 中需要被消费的数据量的大小，以判断数据是否写入完全
        ...
        if (bwr.read_consumed > 0) { // binder 驱动成功返回数据，配置了 mIn 的 dataSize
    ...
    int32_t handle, uint32_t code, const Parcel& data, status_t* statusBuffer)
{
    ...
    // ptr 指向的是 binder 实体（binder_node）在宿主进程的首地址
    tr.target.ptr = 0; /* Don't pass uninitialized stack data to a remote process */
    tr.target.handle = handle; // 目标 server 的 binder 句柄，如果是 0，目标就是 sm
    tr.code = code; // 此处 code 为 TRANSACTION_methodName，比如 addService() 的话就是 TRANSACTION_addService
    ...
    case BR_TRANSACTION:
        {
            ...
            if ((tr.flags & TF_ONE_WAY) == 0) { // flags 为 0，进入 if 分支
