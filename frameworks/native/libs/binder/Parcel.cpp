...
status_t Parcel::flattenBinder(const sp<IBinder>& binder)
{
    ...
    if (binder != nullptr) {
        // Binder.cpp 中 localBinder() 和 remoteBinder() 默认实现都是返回 nullptr
        // BBinder 实现 localBinder()，BpBinder 实现 remoteBinder()
        // 如果是代理对象，保存 handle，否则将 binder 引用保存到 binder 变量中
        // 比如 AMS 注册时，因为 addService 时传入的是 AMS 对象，即服务端，而不是 ams 在客户端的代理，所以就是 BBinder
        BBinder *local = binder->localBinder(); // 返回 BBinder 对象
        if (!local) {
            BpBinder *proxy = binder->remoteBinder(); // 返回 BpBinder 对象
            if (proxy == nullptr) {
                ALOGE("null proxy");
            } else {
...
{
    // val 指向 JavaBBinder 对象，是通过 writeStrongBinder() 传入的参数 Binder 对象（服务对象）转换而来
...
restart_write:
        *reinterpret_cast<flat_binder_object*>(mData+mDataPos) = val;
        ...
        if (nullMetaData || val.binder != 0) {
            // Parcel.mObjects 保存 new XXXService(...) 的服务端
            mObjects[mObjectsSize] = mDataPos;
            // 增加 val(JavaBBinder) 的强引用书以便 flattenBinder() 返回后(强引用会减1) JavaBBinder 对象不被销毁
